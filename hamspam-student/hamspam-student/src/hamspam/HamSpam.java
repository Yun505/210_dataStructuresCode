...
public class HamSpam {
	...
	public String getValue(int n) {
		...
		else if (n%this.hamNumber != 0 && n%this.spamNumber==0) {
			return "spam";
		}
		else if(n%this.hamNumber == 0 && n%this.spamNumber==0){
			return "hamspam";
	...
	public String[] getValues(int start, int end) {
		String myArray[] = new String[end-start+1];
		int increment = start;
		for(int i = 0; i < (end-start+1); i++){
			myArray[i] = getValue(increment);
			increment++;
		}
	...
	// 	HamSpam a = new HamSpam(3, 4);
	// 	String[] b = a.getValues(1,4);
