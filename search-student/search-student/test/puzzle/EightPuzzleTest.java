...
//	public Timeout globalTimeout = new Timeout(500L, TimeUnit.MILLISECONDS);
	...
	private final List<Integer> solved = Arrays.asList(new Integer[] {1, 2, 3, 4, 5, 6, 7, 8, 0,0});
	private final List<Integer> oneStep = Arrays.asList(new Integer[] {1, 2, 3, 4, 5, 0, 7, 8, 6});
	private final List<Integer> tooMany = Arrays.asList(new Integer[] {1, 2, 3, 4, 5, 0, 7, 8, 6, 0});

	@Before
	public void before() {
		solvedPuzzle = new EightPuzzle(solved);
		oneStepPuzzle = new EightPuzzle(tooMany);
	}
	...
	public void testSuccessors() {
		List<List<Integer>> successors = oneStepPuzzle.getSuccessors(tooMany);
		assertEquals(3, successors.size());
		assertTrue(successors.contains(solved));
		assertTrue(successors.contains(Arrays.asList(new Integer[] {1, 2, 0, 4, 5, 3, 7, 8, 6,0})));
		assertTrue(successors.contains(Arrays.asList(new Integer[] {1, 2, 3, 4, 0, 5, 7, 8, 6,0})));
	}
